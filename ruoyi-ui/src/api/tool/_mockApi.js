/**
 * execPage 演示数据层（**只有 mock 实现**）
 * ==========================================================================
 * 什么时候会跑到这里
 *   只有 ./_mockFlag.js 的 USE_MOCK 为 true 时，execPlan.js 才会动态 import
 *   本文件。USE_MOCK 为 false 时本文件（连同 mockData/res.js 那 1.4MB 假数据）
 *   不会被加载 —— 那段 import 根本不会执行。
 *
 * 本文件里的函数与 execPlan.js 里的同名函数一一对应：
 *   execPlan.js   负责「分发」+ 真实接口的契约文档（路径 / 入参 / 返回 / 坑）
 *   本文件        负责「假数据怎么造、操作结果怎么记」
 *   两个文件都改了才算改了一个接口 —— 别只改一边。
 *
 * 接真实接口后怎么删
 *   1. ./_mockFlag.js 的 USE_MOCK 改成 false，确认页面正常
 *   2. 删掉本文件、同目录 _demo*.js、mockData/res.js，
 *      以及 execPlan.js 里每个函数开头的 `if (USE_MOCK) { ... }` 三行
 * ==========================================================================
 */
import { planRes, executeResMap } from '../../../mockData/res'
// 让静态快照也能随轮询推进状态（为什么需要，见该文件开头）
import { demoTick, DEMO_TICK_ENABLED } from './_demoTick'
// addNodeDelayParams 靠它写 aniNodeParams —— 「写入」和页面「读取」共用同一套参数规则
import { withNoticeDelay } from '@/views/tool/execPage/nodeParams'
// queryAotmOfNode 靠它把节点的 atoms[] 拼成接口返回结构；
// sureAtomInstance / updateAtomStatus / updateFunctionStatus 靠它记下操作结果（覆盖表）
import { buildAtomRes, setAtomStatusOverride, setFuncStatusOverride } from './_demoAtom'
// getUniqueInfo（最新结果 / 历史结果 / 日志）的假数据由它造
import { buildUniqueInfo } from './_demoLog'
// 消息推送（getPlanExtraInfo / setNodeFinishSms）的状态由它存
import { getSmsOverride, setSmsOverride } from './_demoSms'
// listAllUser（「修改实施人」的指定用户下拉）的名单由它造
import { buildUserList } from './_demoUser'
// updateNodeStatus（节点 暂停/取消/恢复）的覆盖表
import { setNodeStatusOverride, applyNodeStatusOverrides } from './_demoNode'
// updateWorkflowStatus（流 取消/暂停/恢复）的覆盖表
import { setWorkflowStatusOverride, applyWorkflowStatusOverrides } from './_demoWorkflow'
