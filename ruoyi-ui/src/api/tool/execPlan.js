...
/**
 * 变更执行计划接口（execPage 执行页面 + execPlan 计划列表页）
 * ==========================================================================
 * 演示 / 真实接口的切换
 *   每个函数开头都是同一个套路：
 *
 *     if (USE_MOCK) {
 *       return mockApi('xxx', 参数)   // 走 ./_mockApi.js 里的假数据
 *     }
 *     return request({ ... })        // 走真实后端
 *
 *   开关在 ./_mockFlag.js 的 USE_MOCK，**改一处即整页生效**，不用再注释代码。
 *   mock 实现全部集中在 ./_mockApi.js（按需加载：USE_MOCK 为 false 时不会被加载）。
 *
 * 下面的注释写的都是**真实接口的契约**（路径 / 入参 / 返回 / 已知的坑），
 * mock 特有的说明在 _mockApi.js 里。改一个接口要两个文件一起看。
 * ==========================================================================
 */
 ...
 * _mockApi.js / _demo*.js / mockData/res.js 也就不会被加载
...
function mockApi(name, ...args) {
  return import("./_mockApi").then((mod) => mod[name](...args));
}
 ...
 * （解析规则见 views/tool/execPage/nodeParams.js）。
 *
 * @param {Object} p
